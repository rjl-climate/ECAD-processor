    ...
    pub fn read_stations(&self, path: &Path) -> Result<Vec<StationMetadata>> {
        ...
        for (_line_count, line_result) in reader.lines().enumerate() {
            ...
            if self.skip_headers {
                ...
                if !line.trim_start().chars().next().unwrap_or(' ').is_ascii_digit() {
                    continue;
                }
            }
...
mod tests {
    ...
    fn test_read_stations_file() -> Result<()> {
        ...
        writeln!(temp_file, "STAID, STANAME                                 , CN, LAT    , LON     , HGHT")?;
        writeln!(temp_file, "------,----------------------------------------,---,--------,--------,-----")?;
        writeln!(temp_file, "")?;
    ...
    fn test_read_real_stations_file() -> Result<()> {
        ...
        if !stations.is_empty() {
            println!("First station: ID={}, Name={}", stations[0].staid, stations[0].name);
        }
