...
use crate::utils::constants::{UK_TEMP_MIN_DIR, UK_TEMP_MAX_DIR, UK_TEMP_AVG_DIR, STATIONS_FILE};
use rayon::prelude::*;
...
impl ConcurrentReader {
    ...
    pub async fn read_all_temperature_data(
        &self,
        base_path: &Path,
        ...
        let min_handle: JoinHandle<Result<HashMap<(u32, chrono::NaiveDate), TemperatureRecord>>> = 
            tokio::spawn(async move {
                Self::read_temperature_type_parallel_static(&min_path, stations_clone1, max_workers).await
            });
            
        let max_handle: JoinHandle<Result<HashMap<(u32, chrono::NaiveDate), TemperatureRecord>>> = 
            tokio::spawn(async move {
                Self::read_temperature_type_parallel_static(&max_path, stations_clone2, max_workers).await
            });
            
        let avg_handle: JoinHandle<Result<HashMap<(u32, chrono::NaiveDate), TemperatureRecord>>> = 
            tokio::spawn(async move {
                Self::read_temperature_type_parallel_static(&avg_path, stations_clone3, max_workers).await
            });
        
        // Wait for all reads to complete
        let (min_temps, max_temps, avg_temps) = tokio::try_join!(
            min_handle,
            max_handle,
            avg_handle
        )?;
    ...
    async fn read_temperature_type_parallel_static(
        ...
        let file_prefix = match dir_path.file_name().and_then(|f| f.to_str()) {
            ...
            Some("uk_temp_max") => "TX", 
            Some("uk_temp_avg") => "TG",
            _ => return Err(ProcessingError::InvalidFormat(format!(
                "Unknown temperature directory: {:?}", dir_path
            ))),
        };
        
        let temperature_files: Vec<PathBuf> = Self::find_temperature_files_static(dir_path, &stations, file_prefix)?;
        ...
        for entry in std::fs::read_dir(dir_path)? {
            ...
            if path.is_file() {
                if let Some(file_name) = path.file_name() {
                    ...
                    if file_name_str.starts_with(&format!("{}_STAID", file_prefix)) && file_name_str.ends_with(".txt") {
