...
impl DataMerger {
    ...
    fn group_by_station_and_date(
        ...
        for ((station_id, date), record) in &temperature_data.min_temperatures {
            let entry = grouped.entry((*station_id, *date)).or_insert(TemperatureSet::default());
    ...
    fn create_consolidated_record(
        ...
        if !self.allow_incomplete && (temp_set.min.is_none() || temp_set.max.is_none() || temp_set.avg.is_none()) {
            return Ok(None);
        }
        ...
        let (min_temp, min_quality) = temp_set.min
            .as_ref()
            .map(|r| (r.temperature, r.quality_flag))
            .unwrap_or((-9999.0, 9));
            
        let (max_temp, max_quality) = temp_set.max
            .as_ref()
            .map(|r| (r.temperature, r.quality_flag))
            .unwrap_or((-9999.0, 9));
            
        let (avg_temp, avg_quality) = temp_set.avg
            .as_ref()
            .map(|r| (r.temperature, r.quality_flag))
            .unwrap_or((-9999.0, 9));
...
mod tests {
    ...
    fn test_merge_complete_data() {
        ...
        let records = merger.merge_station_data(
            &station,
            vec![min_temp],
            vec![max_temp],
            vec![avg_temp],
