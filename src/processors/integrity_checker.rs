...
use crate::utils::constants::{MIN_VALID_TEMP, MAX_VALID_TEMP};
...
impl IntegrityChecker {
    ...
    pub fn check_integrity(&self, records: &[ConsolidatedRecord]) -> Result<IntegrityReport> {
        ...
        for record in records {
            station_records.entry(record.station_id)
                .or_default()
                .push(record);
        }
        ...
        for record in records {
            ...
            let stats = report.station_statistics
            ...
            if record.min_temp != -9999.0 {
                stats.min_temp = Some(stats.min_temp.map_or(record.min_temp, |t| t.min(record.min_temp)));
                stats.max_temp = Some(stats.max_temp.map_or(record.min_temp, |t| t.max(record.min_temp)));
            }
        }
    ...
    fn check_record(&self, record: &ConsolidatedRecord, report: &mut IntegrityReport) -> Result<()> {
    ...
    fn check_temperature_ranges(&self, record: &ConsolidatedRecord, report: &mut IntegrityReport) -> Result<()> {
        let temps = [(record.min_temp, "min"), (record.max_temp, "max"), (record.avg_temp, "avg")];
        ...
        for (temp, name) in temps {
            if temp != -9999.0 && !(MIN_VALID_TEMP..=MAX_VALID_TEMP).contains(&temp) {
                report.temperature_violations.push(TemperatureViolation {
                    ...
                    details: format!("{} temperature {} is outside valid range [{}, {}]", 
    ...
    pub fn generate_summary(&self, report: &IntegrityReport) -> String {
        ...
        if !report.temperature_violations.is_empty() {
            ...
            for (i, violation) in report.temperature_violations.iter().take(10).enumerate() {
                summary.push_str(&format!("  {}. Station {} on {}: {}\n", 
                    i + 1, violation.station_id, violation.date, violation.details
                ));
            }
        }
